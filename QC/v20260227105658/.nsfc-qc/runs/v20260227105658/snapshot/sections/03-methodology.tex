...
\textbf{验证方案：}在测试集上对比蒸馏后的轻量路由网络与原多智能体系统的性能差距，要求PSNR损失$\leq 5\%$、SSIM损失$\leq 3\%$。测试部署时延（ms/图像）、模型参数量（MB）、计算量（FLOPs），验证在Jetson Nano或同等边缘设备上的实时推理可行性（目标：$\geq 10$ FPS @ 720p）。通过消融实验验证：（1）多任务联合训练 vs 单任务独立训练；（2）困难样本再训练 vs 单阶段训练；（3）知识蒸馏损失 vs 硬标签监督。

\textbf{训练与推理区别：}训练阶段使用离线轨迹监督并联合优化多任务损失；推理阶段仅执行前向路由预测与工具调用，避免大模型在线推理开销。

\textbf{风险与回退策略：}针对分割误差传播问题，设置区域置信度门控与候选掩码回退；针对规划误差累积问题，引入全局一致性约束与早停机制；针对蒸馏偏差问题，采用高质量轨迹重加权与困难样本再训练。

\subsection{研究计划}

研究计划按"阶段目标—里程碑—可交付件—验收标准"组织如下：

\textbf{第一阶段（第1–4月）：空间感知模块研发}

阶段目标：完成空间感知退化分析机制构建，实现区域级退化识别与量化。

里程碑：完成SAM分割与VLM区域退化识别联调，形成空间退化图生成流程；建立区域级退化识别评测基准。

可交付件：空间感知模块代码库、区域级标注与评测脚本、阶段实验报告。

验收标准：在SIDD、GoPro、NH-HAZE等公开基准上完成区域级退化识别基线对比，关键指标（退化类型识别准确率、严重程度估计误差）优于全图级识别基线；空间退化图质量评分达到预设阈值。

\textbf{第二阶段（第5–8月）：层级多智能体协同规划系统研发}

阶段目标：完成层级多智能体协同机制构建，实现区域级恢复规划与连续参数自适应优化。

里程碑：打通全局调度器、区域专家、质量反馈闭环；完成工具链选择与参数优化的一体化决策流程。

可交付件：层级智能体恢复系统原型、消融实验结果、第一篇论文初稿、发明专利交底书。

验收标准：在混合退化任务上相较AirNet、PromptIR等静态管线取得质量（PSNR提升$\geq$0.5dB）或效率（推理时间降低$\geq$20\%）改进；完成与AgenticIR、MAIR等智能体方法的对比分析。

\textbf{第三阶段（第9–12月）：轨迹蒸馏与轻量部署}

阶段目标：完成轨迹蒸馏驱动的轻量路由网络构建，实现高质量恢复策略的快速推理与边缘部署。

里程碑：构建规模不少于10000条的多智能体决策轨迹数据集；完成轻量路由网络训练与端侧部署测试。

可交付件：轻量路由网络模型、部署测试报告、第二篇论文与专利申请材料、可复现实验配置。

验收标准：在主要质量指标（PSNR、SSIM）可接受损失范围（$\leq$5\%）内，部署时延降低$\geq$50\%、算力开销（FLOPs）降低$\geq$60\%；完成边缘设备（如Jetson Nano或同等级嵌入式平台）上的实时推理验证。

时间映射关系：研究内容1对应第一阶段，研究内容2对应第二阶段，研究内容3对应第三阶段；各阶段输出将逐步汇聚为最终系统。阶段间的依赖关系为：第一阶段输出$\mathcal{M}$作为第二阶段输入；第二阶段运行产生轨迹数据作为第三阶段输入。
