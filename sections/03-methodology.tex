...
本项目的整体研究思路为“感知$\to$规划$\to$执行$\to$蒸馏”。首先构建空间感知模块实现区域级退化分析，然后设计层级智能体进行协同规划与执行，最后通过轨迹蒸馏实现轻量化部署。具体方法流程如图~\ref{fig:method-pipeline}~所示。

\vspace{6pt}
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{imgs/main.pdf}\\[2pt]
\caption{具体研究方法流程}
...
\textbf{验证方案：}在SIDD去噪、GoPro去模糊、NH-HAZE去雾等公开基准上，采用分类准确率（Acc）、平均精度（mAP）和F1分数评估退化识别性能。对比方案包括：（1）全图级DepictQA分析基线；（2）SAM分割 + 独立区域分析（无上下文聚合）；（3）本研究完整方法。通过消融实验量化各组件（RobustSAM、上下文聚合）对最终恢复质量（PSNR/SSIM）的贡献。
...
\textbf{验证方案：}在混合退化合成数据集（通过SIDD、GoPro、NH-HAZE等组合生成）与真实测试样本上，与AirNet、PromptIR等AiO静态方法以及AgenticIR、MAIR等智能体方法进行全面对比。评估指标包括：（1）恢复质量：PSNR、SSIM、LPIPS；（2）推理效率：FPS、平均工具调用次数；（3）决策质量：工具链选择准确率、参数优化收敛速度。通过消融实验验证层级架构各组件（全局调度、区域专家、质量反馈、一致性约束）的贡献度。
